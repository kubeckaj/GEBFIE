...
# ------------------------------------------------------------
JOBDIR2=$(echo $JOBDIR | sed "s/REACTANTS/PRODUCTS/")
SCRDIR=/scratch/${USER}/TM_${QUEUE}_${JOB_ID}
mkdir -p ${SCRDIR}
cp $JOBDIR2/input.sh $SCRDIR/input.sh
cp $JOBDIR2/structure.xyz $SCRDIR/structure.xyz
cp -r $JOBDIR/CHECKPOINTS/ $SCRDIR/
cd ${SCRDIR}
echo ${PWD}
